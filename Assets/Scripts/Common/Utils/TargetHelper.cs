...
public class TargetHelper : Singleton<TargetHelper>
{
    ...
    public List<GameObject> SearchTargets(GameObject from, TargetType targetType) {
        ...
        switch (targetType) {
            ...
            case TargetType.ALLIES: {
                if (partyType == PartyType.Player) {
                    List<GameObject> catObjs = ConvertToGameObject(self.playerParty.cats.Cast<Creature>().ToList());
                    ...
                    if (self == self.enemyParty.boss) {
                        if (self.enemyParty.creeps != null && self.enemyParty.creeps.Count > 0) {
                            List<GameObject> creeps = ConvertToGameObject(self.enemyParty.creeps.Cast<Creature>().ToList());
    ...
    private List<GameObject> ConvertToGameObject(List<Creature> creatures) {
        if (gameObject != null) {
